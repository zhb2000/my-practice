...
//Get k-th number
int rank_k(int u, int k)
{
	assert(u != 0);
	assert(k >= 1);
	int lns = num_size(ns[u].lch);
	int rns = num_size(ns[u].rch);
	if (k <= lns)
