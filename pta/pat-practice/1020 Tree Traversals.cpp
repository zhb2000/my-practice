#ifndef LOCAL
#define NDEBUG
#endif // close assert if not local
#include<iostream>
#include<cstdio>
#include<string>
#include<cstring>
#include<functional>
#include<algorithm>
#include<vector>
#include<map>
#include<unordered_map>
#include<set>
#include<unordered_set>
#include<stack>
#include<queue>
#include<deque>
#include<utility>
#include<tuple>
#include<bitset>
#include<cassert>
#include<cctype>
#include<cmath>
#include<ctime>
using namespace std;
typedef long long ll;
typedef unsigned long long ull;
typedef unsigned int uint;
typedef pair<int, int> Pii;
template <class Int>
inline Int fast_read()
{
	Int x = 0, f = 1; char ch = char(getchar());
	while (!isdigit(ch)) { if (ch == '-') f = -1; ch = char(getchar()); }
	while (isdigit(ch)) { x = x * 10 + ch - '0'; ch = char(getchar()); }
	return x * f;
}
inline int readint() { return fast_read<int>(); }
inline ll readll() { return fast_read<ll>(); }
inline void cpp_io() { ios::sync_with_stdio(false); cin.tie(nullptr); cout.tie(nullptr); }
template <class T>
ostream &operator<<(ostream &os, const vector<T> &vec)
{
	os << "["; bool isfirst = true;
	for (const auto &x : vec)
	{
		if (isfirst) isfirst = false;
		else os << ", ";
		os << x;
	}
	os << "]"; return os;
}
template <class T1, class T2>
ostream &operator<<(ostream &os, const pair<T1, T2> &p)
{
	os << "(" << p.first << ", " << p.second << ")"; return os;
}
template <class Int>
inline Int avg2(Int a, Int b) { assert(a <= b); return a + ((b - a) >> 1); }
template <class Int>
inline Int clen(Int l, Int r) { assert(l - 1 <= r); return r - l + 1; }
template <class Int>
inline bool isodd(Int num) { return (num & 1) != 0; }
template <class Int>
inline bool iseven(Int num) { return (num & 1) == 0; }
const int INTMAX = ~(1 << 31);
const double EPS = 1e-6;
//------------------------ header end ------------------------

struct Node
{
	int val;
	int lch;
	int rch;
};

const int maxn = 100;
Node ns[maxn];
int node_cnt = 0;
int new_node() { return ++node_cnt; }
int postorder[maxn];
int inorder[maxn];

Pii lr_size(int il, int ir, int root_val)
{
	for (int i = il; i <= ir; i++)
	{
		if (inorder[i] == root_val)
		{
			int lsz = clen(il, i - 1);
			int rsz = clen(i + 1, ir);
			return { lsz, rsz };
		}
	}
	assert(false);
	return { -1, -1 };
}

int build_tree(int pl, int pr, int il, int ir)
{
	//printf("post: [%d, %d], in: [%d, %d]\n", pl, pr, il, ir);
	assert(clen(pl, pr) == clen(il, ir));
	if (pr < pl)
		return -1;
	int root_val = postorder[pr];
	int root = new_node();
	ns[root].val = root_val;
	int lsz, rsz;
	tie(lsz, rsz) = lr_size(il, ir, root_val);
	int lch = build_tree(pl, pl + lsz - 1, il, il + lsz - 1);
	int rch = build_tree(pl + lsz, pr - 1, il + lsz + 1, ir);
	ns[root].lch = lch;
	ns[root].rch = rch;
	return root;
}


int main()
{
#ifdef LOCAL
	freopen("in.txt", "r", stdin);
	//freopen("out.txt", "w", stdout);
#endif

	//cpp_io();
	int n;
	cin >> n;
	for (int i = 1; i <= n; i++)
		cin >> postorder[i];
	for (int i = 1; i <= n; i++)
		cin >> inorder[i];
	int root = build_tree(1, n, 1, n);
	bool isfirst = true;
	queue<int> q;
	q.push(root);
	while (!q.empty())
	{
		int u = q.front(); q.pop();
		if (isfirst)
			isfirst = false;
		else
			cout << " ";
		cout << ns[u].val;
		if (ns[u].lch != -1)
			q.push(ns[u].lch);
		if (ns[u].rch != -1)
			q.push(ns[u].rch);
	}


#ifdef CONSOLE_PAUSE
	system("PAUSE");
#endif
	return 0;
}
